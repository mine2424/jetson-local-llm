...
GPU ビルドがない場合 (`libggml-cuda.so` が存在しない)、すべてのスクリプトが自動的に CPU モードに切り替わります。

```bash
# CPU モードの確認
ls ~/llama.cpp/build/bin/libggml-cuda.so* 2>/dev/null \
  && echo "GPU ビルドあり" || echo "CPU 専用ビルド"

# CPU モードで起動 (自動検出 — 追加フラグ不要)
bash scripts/llama-server-optimized.sh /path/to/model.gguf
# → モード: CPU (-ngl 0)  ← ~7 t/s と表示される

# TUI から起動する場合
# Service → L1. LFM-2.5 llama-server 起動
# → libggml-cuda.so がなければ自動で CPU モードになる
```
...
|-------|-----------|-----------|
| `-ngl` | `999` (全レイヤー) | `0` (自動) |
| `--flash-attn` | ON | OFF (CUDA専用) |
| `--cache-type-k/v` | `q8_0` | デフォルト (f16) |
| `-t` | `6` | `6` (同じ) |
| 速度目安 | 40〜60 t/s | ~7 t/s |

**CPU ビルドのまま使う場合は `-c` を小さくするとメモリを節約できます:**
...
**GPU ビルドにアップグレードしたい場合:**

```bash
# TUI: Setup → 4. llama.cpp ビルド単体
# または直接:
